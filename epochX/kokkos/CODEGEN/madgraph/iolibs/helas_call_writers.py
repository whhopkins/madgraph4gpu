...
"""
    ...
    def get_external_line(self, wf, argument):
        ...
        if wf.get('mass').lower() != 'zero' or argument.get('spin') != 2: 
            ...
            call = call + "(Kokkos::subview(local_mom,%d,Kokkos::ALL),"
            if argument.get('spin') != 1:
                # For non-scalars, need mass and helicity
                call = call + "pars->%s, cHel(%d),"
            else:
                call = call + "pars->%s,"
            call = call + "%+d,w[%d]);"
            if argument.get('spin') == 1:
                ...
                misc.sprint(call)
                misc.sprint( (wf.get('number_external')-1,
                                 wf.get('mass'),
                                 wf.get('number_external')-1,
                                 # For boson, need initial/final here
                                 (-1) ** (wf.get('state') == 'initial'),
            ...
            # Specify namespace for Helas calls
            ##call = call + "((double *)(dps + %d * dpt),"
            call = call + "(Kokkos::subview(local_mom,%d,Kokkos::ALL), cHel(%d),%+d,w[%d]);"
    ...
    def get_matrix_element_calls(self, matrix_element, color_amplitudes):
        ...
        res = []
        # reset jamp:
        res.append('for(int i=0;i<%s;i++){jamp[i] = complex_t<double>(0.,0.);}'
