        ...
        if args and args[0][0] != '-':
            ...
            if path == 'auto' and self._export_format in \
                     ['madevent', 'standalone', 'standalone_cpp', 'matchbox_cpp', 'madweight',
                      ...
                      # WH: adding this in before completely understand this
                      'standalone_sycl', 'standalone_kokkos'
                      ]:
    ...
    def get_default_path(self):
        ...
        if self._export_format == 'NLO':
            ...
            if self._export_format == 'standalone_cpp':
                ...
                auto_path = lambda i: pjoin(self.writing_dir,
                                               name_dir(i))
            elif self._export_format == 'standalone_sycl':
                name_dir = lambda i: 'PROC_SA_SYCL_%s_%s' % \
                                    (self._curr_model['name'], i)
                auto_path = lambda i: pjoin(self.writing_dir,
                                               name_dir(i))
            elif self._export_format == 'standalone_kokkos':
                name_dir = lambda i: 'PROC_SA_KOKKOS_%s_%s' % \
                                    (self._curr_model['name'], i)
                auto_path = lambda i: pjoin(self.writing_dir,
                                               name_dir(i))
            else:
...
class MadGraphCmd(HelpToCmd, CheckValidForCmd, CompleteForCmd, CmdExtended):
    ...
    def setup(self):
        ...
        self._export_formats = self._v4_export_formats + ['standalone_cpp', 'pythia8', 'standalone_gpu','standalone_sycl', 'standalone_kokkos']
...
#        line_options = dict(arg[2:].split('=') for arg in args if arg.startswith('--') and '=' not in arg)
        ...
        config['standalone_sycl'] = {'check': False, 'exporter': 'cpp', 'output': 'Template'}
        config['standalone_kokkos'] = {'check': False, 'exporter': 'cpp', 'output': 'Template'}
...
""")
        ...
        elif options['exporter'] in ['cpp', 'gpu', 'kokkos', 'sycl']:
            ...
            if options['me_exporter']['exporter'] == 'v4':
                with misc.TMP_variable(self, '_export_format', options['me_exporter']['name']):
                    self._me_curr_exporter = export_v4.ExportV4Factory(self, noclean, 
                                                group_subprocesses=group_processes,
                                                cmd_options=line_options)
            elif options['me_exporter']['exporter']  in ['cpp','gpu', 'kokkos', 'sycl']:
                # check for PLUGIN format
                output_cls = misc.from_plugin_import(self.plugin_path, 'new_output',
    ...
    def export(self, nojpeg = False, main_file_name = "", group_processes=True, 
        ...
        elif self._curr_exporter.exporter == 'kokkos':       
            self._curr_helas_model = helas_call_writers.KOUFOHelasCallWriter(self._curr_model)
        elif self._curr_exporter.exporter == 'sycl':       
            self._curr_helas_model = helas_call_writers.SYFOHelasCallWriter(self._curr_model)
        ...
        if self._me_curr_exporter:
            ...
            if self._curr_exporter.exporter == self._me_curr_exporter.exporter:
                ...
                elif self._me_curr_exporter.exporter == 'kokkos':       
                    self._me_curr_helas_model = helas_call_writers.KOUFOHelasCallWriter(self._curr_model)
                elif self._me_curr_exporter.exporter == 'sycl':       
                    self._me_curr_helas_model = helas_call_writers.SYUFOHelasCallWriter(self._curr_model)
